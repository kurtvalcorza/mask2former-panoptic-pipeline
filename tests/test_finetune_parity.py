"""NOTEBOOK_SPEC 2.0 parity tests (PAR1–PAR3) for the standalone E2E fine-tuning notebook.
...
TEMPLATE = _load("notebook_template_finetune").TEMPLATE
...
def test_par3_generator_check_is_clean(notebook: dict) -> None:
    ...
    assert current == rendered, (
        "notebook is stale; run python tools/build_notebook.py --template tools/notebook_template_finetune.py"
    )
...
def test_finetune_notebook_declares_e2e_and_adapts_from_the_verified_snapshot(notebook: dict) -> None:
    meta = notebook["metadata"]["dimer"]
    assert meta["notebook_profile"] == "E2E" and meta["notebook_mode"] == "GUIDED"
    first = _source(notebook["cells"][0])
    assert "**Run all:**" in first and "**runs the bounded fine-tune**" in first
    # Stage cells only: the carried modules' own text is exempt (their error messages name the flag).
    stage = "\n".join(
        _source(c)
        for c in _cells(notebook, "code")
        if not c.get("metadata", {}).get("dimer", {}).get("embedded_module")
    )
    # After Section 3 nothing may reach for the Hub: the only allow_download=True is the generator's staging call,
    # and the re-headed adapter is built from the already-verified WEIGHTS_DIR.
    assert stage.count("allow_download=True") == 1
    assert "stage_missing_files(WEIGHTS_DIR, allow_download=True)" in stage
    assert (
        "from_pretrained(weights_dir=WEIGHTS_DIR, class_names=SHAPE_CLASSES, stuff_names=SHAPE_STUFF, seed=SEED)"
        in stage
    )
    assert "run = adapter.finetune(" in stage and "load_artifact(ARTIFACT_DIR)" in stage
    # FT2: the adaptation is on the default path, not behind a gate.
    for gate in ("USE_BYOD_IMAGE", "USE_BYOD_DATASET"):
        assert f"{gate} = False  # @param" in stage


def test_both_notebooks_share_the_package_and_the_pins() -> None:
